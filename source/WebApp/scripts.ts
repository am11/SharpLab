...
const asm = task('asm', async () => {
    ...
    const outputContents = [ "// This file is generated by 'asm' task defined in 'WebPack/scripts.ts'",
        `export const instructionsRegex = new RegExp(${regexPattern});` ].join('\r\n');

    await jetpack.writeAsync(asmOutputPath, outputContents);
}, {
    watch: [ `${dirname}/components/internal/codemirror/mode-asm-instructions.txt` ]
});
